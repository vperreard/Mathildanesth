...
export default function HomePage() {
    ...
    if (!mounted)
        return null;
    return (<div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-primary-50">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {!isLoading && isAdmin && (<motion.div initial="hidden" animate="visible" variants={fadeIn}>
                        <AdminRequestsDashboard />
                    </motion.div>)}

                {/* Features Grid */}
                <div className={"".concat(user ? 'py-8' : 'py-16')}>
                    <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {featureCards.map(function (card, idx) { return (<motion.div key={idx} variants={fadeIn} custom={idx} whileHover="hover" initial="rest" animate="rest" className="group">
                                <motion.div variants={cardHover} className="h-full bg-white/80 backdrop-blur-sm rounded-2xl shadow-soft p-6 border border-gray-100 flex flex-col">
                                    <div className={"w-12 h-12 bg-gradient-to-r ".concat(card.gradient, " rounded-xl flex items-center justify-center mb-5 shadow-sm group-hover:shadow-md transition-all duration-300")}>
                                        {card.icon}
                                    </div>
                                    <h3 className={"text-lg font-semibold ".concat(card.color, " mb-3")}>{card.title}</h3>
                                    <p className="text-gray-600 mb-5">{card.description}</p>
                                    <div className="mt-auto">
                                        <Link href={card.href} className={"inline-flex items-center ".concat(card.color, " hover:opacity-80 font-medium text-sm")}>
                                            Accéder
                ...
                <div className="py-16">
                    ...
                    <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                        {otherFeatures.map(function (feature, idx) { return (<motion.div key={idx} variants={fadeIn} whileHover="hover" initial="rest" animate="rest">
                                <motion.div variants={cardHover}>
                                    <Link href={feature.href} className="group flex items-start p-6 border border-gray-100 rounded-xl hover:bg-white hover:shadow-soft transition-all duration-300">
