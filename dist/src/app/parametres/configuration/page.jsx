...
import { Settings, Calendar, FileText, Award, Activity, Grid, Share2, Layout, TableProperties } from 'lucide-react';
...
var ConfigurationPanelPage = function () {
    var _a = useState('types'), selectedItem = _a[0], setSelectedItem = _a[1];
    var menuItems = [
        {
            id: 'types',
            ...
            <aside className="w-1/4 p-6 border-r bg-white border-gray-200">
                ...
                <nav>
                    <ul className="space-y-2">
                        {menuItems.map(function (item) { return (<li key={item.id}>
                                <Button variant={selectedItem === item.id ? "primary" : "ghost"} onClick={function () { return setSelectedItem(item.id); }} fullWidth className={cn("justify-start py-2.5 px-4 hover:bg-gray-100 transition-colors", selectedItem === item.id && "text-white")}>
