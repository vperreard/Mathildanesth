...
describe('TrameModelesConfigPanel', () => {
    beforeEach(() => {
        ...
        mockedUseAuth.mockReturnValue({
            user: { id: '1', email: 'test@example.com', role: 'ADMIN_TOTAL', prenom: 'Test', nom: 'User' },
    ...
    test('devrait afficher le titre principal et le bouton "Ajouter un modèle" lorsque authentifié', async () => {
        ...
        expect(screen.getByRole('button', { name: /Ajouter un modèle/i })).toBeInTheDocument();
    });
    ...
    test('devrait afficher un message de non authentifié si l'utilisateur n'est pas connecté', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText('Vous devez être connecté pour gérer les modèles de trame.')).toBeInTheDocument();
        });
...
describe('TrameModelesConfigPanel - Affichage des données', () => {
    ...
    test('devrait afficher les modèles de trame dans le tableau', async () => {
        ...
        mockedAxios.get.mockImplementation((url) => {
            ...
            if (url === '/api/sites') return Promise.resolve({ data: [{ id: 'site-1', name: 'Site A' }, { id: 'site-2', name: 'Site B' }] });
...
describe('TrameModelesConfigPanel - Modale TrameModele', () => {
    beforeEach(() => {
        ...
        mockedAxios.get.mockImplementation((url) => {
            if (url === '/api/trame-modeles?includeAffectations=true') return Promise.resolve({ data: [] });
            if (url === '/api/sites') return Promise.resolve({ data: [{ id: 'site-1', name: 'Site Test' }] }); // Fournir un site pour le select
            if (url === '/api/activity-types') return Promise.resolve({ data: [] });
            if (url === '/api/operating-rooms') return Promise.resolve({ data: [] });
            if (url === '/api/admin/professional-role-configs') return Promise.resolve({ data: [] });
            if (url === '/api/specialties') return Promise.resolve({ data: [] });
    ...
    test('devrait ouvrir la modale d\'ajout de TrameModele avec les champs vides lorsque le bouton "Ajouter un modèle" est cliqué', async () => {
        render(<TrameModelesConfigPanel />);

        // Attendre que le bouton soit disponible après le chargement initial
        const addButton = await screen.findByRole('button', { name: /Ajouter un modèle/i });
