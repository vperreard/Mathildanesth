...
export async function PUT(
  ...
  try {
    ...
    if (existingRule.metadata?.isDefault && !canManageSystemRules(authContext)) {
      return NextResponse.json({ error: 'Cannot modify system rules' }, { status: 403 });
    }
    ...
    const hasSignificantChanges =
      data.conditions !== undefined || data.actions !== undefined || data.type !== undefined;
    ...
    try {
      ...
      const changeType =
        data.status === 'active' && existingRule.status !== 'active'
          ? 'activated'
      ...
      logger.error('Failed to send rule change notification:', error);
    }
