...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!data.actionType) {
      return NextResponse.json({ error: "Le type d'action est requis" }, { status: 400 });
    }
    ...
    if (!existingConflict) {
      return NextResponse.json({ error: 'Conflit non trouvé ou déjà résolu' }, { status: 404 });
    }
    ...
    switch (data.actionType) {
      case 'MODIFY_RULES':
        ...
        if (
          !data.modifiedRules ||
          !Array.isArray(data.modifiedRules) ||
          data.modifiedRules.length === 0
        ) {
      ...
      case 'DELETE_RULES':
        ...
        if (
          !data.rulesToDelete ||
          !Array.isArray(data.rulesToDelete) ||
          data.rulesToDelete.length === 0
        ) {
          return NextResponse.json(
      ...
      default:
        return NextResponse.json({ error: "Type d'action non valide" }, { status: 400 });
    }
