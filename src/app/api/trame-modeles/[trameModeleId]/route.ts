...
import {
  ...
  RecurrenceTypeTrame,
  TypeSemaineTrame,
  TrameRoleType as PrismaTrameRoleType,
} from '@prisma/client';
  ...
  try {
    ...
    const trameModele = await prisma.trameModele.findUnique({
      ...
      include: {
        site: true,
        affectations: {
          // Inclure les affectations et leurs détails
          include: {
            activityType: true,
            operatingRoom: true,
            personnelRequis: {
...
export async function PUT(
  ...
  try {
    ...
    if (
      ...
      !authResult.role ||
      !['ADMIN_TOTAL', 'ADMIN_PARTIEL'].includes(authResult.role)
    ) {
      return NextResponse.json(
    ...
    if (data.joursSemaineActifs !== undefined) {
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              'joursSemaineActifs doit être un tableau de nombres uniques entre 1 (Lundi) et 7 (Dimanche).',
          },
          { status: 400 }
        );
      }
    ...
    if (data.detailsJson !== undefined) {
      ...
      try {
        ...
        return NextResponse.json(
          { error: 'Le champ detailsJson doit être un objet JSON valide.' },
          { status: 400 }
        );
      }
    }
    ...
    if (data.name) {
      ...
      if (existingTrameWithName) {
        return NextResponse.json(
          { error: 'Un autre template de trameModele avec ce nom existe déjà.' },
          { status: 409 }
        );
      }
    }
    ...
    if (error.code === 'P2025') {
      ...
      return NextResponse.json(
        { error: 'Modèle de trameModele non trouvé pour la mise à jour.' },
        { status: 404 }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (error.code === 'P2025') {
      ...
      return NextResponse.json(
        { error: 'Modèle de trameModele non trouvé pour la suppression.' },
        { status: 404 }
      );
    }
    return NextResponse.json(
