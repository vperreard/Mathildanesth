...
export function OperatingSectorList() {
    ...
    const { data: sectors = [], isLoading: isLoadingSectors, error: sectorsError } = useOperatingSectorsQuery();
    ...
    const handleFormSubmit = async (formData: Omit<OperatingSector, 'id' | 'salles'>, id?: string) => {
        try {
            if (id) {
    ...
    const isLoading = isLoadingSectors || createMutation.isPending || updateMutation.isPending || deleteMutation.isPending;
    ...
    return (
        <div className="p-4 space-y-4">
            ...
            {sectorsError && (
                <div className="text-red-600 bg-red-100 p-3 rounded-md">Erreur de chargement des secteurs: {sectorsError.message}</div>
            )}
                        ...
                        {sectors.map((sector) => (
                            <TableRow key={sector.id}>
                                ...
                                <TableCell className="text-right">
                                    <button onClick={() => handleOpenForm(sector)} disabled={isLoading} aria-label="Modifier" className="p-2 hover:bg-gray-100 rounded">
                                        <PencilIcon className="h-4 w-4" />
                                    </button>
                                    <button onClick={() => openDeleteDialog(sector)} disabled={isLoading} aria-label="Supprimer" className="p-2 hover:bg-gray-100 rounded">
            ...
            <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Êtes-vous sûr de vouloir supprimer le secteur "{sectorToDelete?.nom}"? Cette action est irréversible.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
