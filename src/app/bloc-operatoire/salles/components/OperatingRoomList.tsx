...
export function OperatingRoomList() {
    ...
    const { data: rooms = [], isLoading: isLoadingRooms, error: roomsError } = useOperatingRoomsQuery();
    ...
    const isLoading = isLoadingRooms || isLoadingSectors || createMutation.isPending || updateMutation.isPending || deleteMutation.isPending;
    ...
    return (
        <div className="p-4 space-y-4">
            <div className="flex justify-between items-center">
                ...
                <button onClick={() => handleOpenForm()} disabled={isLoading} className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2">
                    <PlusIcon className="mr-2 h-4 w-4" /> Ajouter une Salle
                </button>
            </div>

            {roomsError && (
                <div className="text-red-600 bg-red-100 p-3 rounded-md">Erreur de chargement des salles: {roomsError.message}</div>
            )}
                        ...
                        {rooms.map((room) => (
                            <TableRow key={room.id}>
                                ...
                                <TableCell className="text-right">
                                    ...
                                    <button onClick={() => handleOpenForm(room)} disabled={isLoading} aria-label="Modifier" className="p-2 hover:bg-gray-100 rounded">
                                        <PencilIcon className="h-4 w-4" />
                                    </button>
                                    <button onClick={() => openDeleteDialog(room)} disabled={isLoading} aria-label="Supprimer" className="p-2 hover:bg-gray-100 rounded">
            ...
            <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Êtes-vous sûr de vouloir supprimer la salle "{roomToDelete?.nom} ({roomToDelete?.numero})"? Cette action est irréversible.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel onClick={closeDeleteDialog} disabled={deleteMutation.isPending}>Annuler</AlertDialogCancel>
                        <AlertDialogAction
