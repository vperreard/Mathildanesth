...
describe('API Load Testing', () => {
  ...
  beforeAll(async () => {
    ...
    global.fetch = jest.fn().mockImplementation(async (url: string, options?: any) => {
      ...
      if (options?.method === 'POST' && url.includes('/api/conges')) {
        return {
          ok: true,
          status: 201,
          json: async () => ({ id: 'mock-leave-id', success: true }),
        };
      }

      if (options?.method === 'POST' && url.includes('/api/planning/generate')) {
        return {
          ok: true,
          status: 200,
          json: async () => ({ planning: 'mock-planning-data' }),
        };
      }

      // Pour tous les autres endpoints GET
      return {
        ok: true,
        status: 200,
  ...
  describe('GET /api/planning/generate - Endpoint complexe', () => {
    it('should generate planning within acceptable time limits', async () => {
      ...
      expect(response.status).toBe(200);
      expect(responseTime).toBeLessThan(1000); // Ajusté pour mocks
