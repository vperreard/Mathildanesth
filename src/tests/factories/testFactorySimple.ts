...
export class TestFactory {
  ...
  static LeaveBalance = {
    ...
    createForUser: (userId: number, year: number, overrides: any = {}) => ({
      id: overrides.id || `balance-${userId}-${year}`,
      userId: userId,
      leaveType: overrides.leaveType || 'ANNUAL',
      year: year,
      initial: overrides.initial || 30,
      total: overrides.total || overrides.initial || 30,
      used: overrides.used || 0,
      pending: overrides.pending || 0,
      remaining:
        overrides.remaining ||
        (overrides.initial || 30) - (overrides.used || 0) - (overrides.pending || 0),
      createdAt: overrides.createdAt || new Date(),
      updatedAt: overrides.updatedAt || new Date(),
      ...overrides,
    }),
