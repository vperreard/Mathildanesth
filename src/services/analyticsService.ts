 ...
 *
 * @description Analyse la répartition des gardes et astreintes entre les membres de l'équipe
 * pour identifier les déséquilibres potentiels et assurer une distribution équitable.
 * Le score pondéré prend en compte le type de garde et sa durée.
 *
 * @returns {Promise<GuardDutyDistribution[]>} Liste triée par score pondéré décroissant
 *
 * @example
 * const distribution = await getGuardDutyDistributionStats();
 * // Retourne: [
 * //   { userId: "1", userName: "Jean Martin", guardCount: 12, onCallCount: 8, totalHours: 220, weightedScore: 276 },
 * //   { userId: "2", userName: "Sophie Bernard", guardCount: 11, onCallCount: 11, totalHours: 198, weightedScore: 242.5 }
 * // ]
 *
 ...
 *
 * @description Identifie les périodes de forte demande de congés en analysant les 12 derniers mois.
 * Corrèle ces pics avec les périodes de vacances scolaires et jours fériés pour prédire
 * les futures périodes de tension et améliorer la planification des ressources.
 *
 * @returns {Promise<LeavePeakAnalysis[]>} Analyse mensuelle des demandes de congés
 *
 * @example
 * const analysis = await getLeavePeakAnalysis();
 * // Retourne l'analyse des 12 derniers mois avec:
 * // - Nombre de demandes par période
 * // - Taux d'approbation
 * // - Indicateurs de vacances scolaires/jours fériés
 * // - Tendance (croissant/stable/décroissant)
 *
 * @throws {Error} En cas d'erreur de connexion à la base de données
 *
 * @todo Implémenter la connexion réelle avec publicHolidayService
 * @todo Ajouter l'analyse prédictive basée sur l'historique
 */
