...
export class PlanningService {
    ...
    static async saveAssignments(assignments: Assignment[]): Promise<boolean> {
        try {
            ...
            const response = await fetch('/api/assignments', { // Supposons une route PATCH ou POST pour la sauvegarde
    ...
    static async validateAssignments(assignments: Assignment[]): Promise<RuleViolation[]> {
        try {
            ...
            const response = await fetch('/api/assignments/validate', { // Supposons une route POST pour la validation
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ assignments })
            });
