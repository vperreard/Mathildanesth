...
/**
 * Represents a planning slot for user assignments
 */
...
/**
 * Service handling planning operations including assignment validation and persistence
 */
export class PlanningService {
  /**
   * Saves assignments to the database via API
   * @param attributions - Array of assignments to save
  ...
  static async saveAssignments(attributions: Attribution[]): Promise<boolean> {
    try {
      ...
      const response = await fetch('http://localhost:3000/api/gardes/vacations', {
        // Supposons une route PATCH ou POST pour la sauvegarde
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || 'Erreur lors de la sauvegarde des gardes/vacations via API'
        );
      }
  ...
  /**
   * Validates assignments against business rules via API
   * @param attributions - Array of assignments to validate
   * @returns Promise resolving to array of rule violations (empty if valid)
   */
  static async validateAssignments(attributions: Attribution[]): Promise<RuleViolation[]> {
    try {
      ...
      const response = await fetch('http://localhost:3000/api/gardes/vacations/validate', {
        // Supposons une route POST pour la validation
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ attributions }),
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || 'Erreur lors de la validation des gardes/vacations via API'
        );
      }
  ...
  static async getAssignments(startDate: Date, endDate: Date): Promise<Attribution[]> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || 'Erreur lors de la récupération des gardes/vacations via API'
        );
      }
      const data = await response.json();
