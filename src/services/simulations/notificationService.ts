...
import type { SimulationProgress } from '@/types/simulation-notifications';
...
export class SimulationNotificationService {
  ...
  async notifySimulationStarted(scenarioId: string, userId: string, estimatedDuration?: number) {
    try {
      ...
      await this.updateSimulationStatus(scenarioId, 'running', 0, 'Simulation démarrée');

      // 4. Envoyer une notification temps réel via Pusher
      const channelName = `private-user-${userId}`;
      await pusherServer.trigger(channelName, SimulationEvent.STARTED, {
        scenarioId,
  ...
  async notifySimulationProgress(scenarioId: string, userId: string, progress: SimulationProgress) {
    try {
      ...
      const channelName = `private-user-${userId}`;
      await pusherServer.trigger(channelName, SimulationEvent.PROGRESS, {
        ...progress,
        timestamp: new Date().toISOString(),
      });
  ...
  async notifySimulationCompleted(
    ...
    try {
      ...
      // 3. Mettre à jour le statut de la simulation
      await this.updateSimulationStatus(scenarioId, 'completed', 100, 'Simulation terminée');

      // 4. Envoyer une notification temps réel via Pusher
      const channelName = `private-user-${userId}`;
      await pusherServer.trigger(channelName, SimulationEvent.COMPLETED, {
        scenarioId,
        scenarioName: scenario.name,
        resultId,
        executionTime: executionTimeSeconds,
      ...
      const isPublic =
        (await prisma.simulationScenario.count({
          where: {
            id: scenarioId,
            isPublic: true,
          },
  ...
  async notifySimulationError(scenarioId: string, userId: string, error: string) {
    try {
      // 1. Récupérer le scénario pour avoir son nom
      const scenario = await prisma.simulationScenario.findUnique({
        where: { id: scenarioId },
      ...
      // 3. Mettre à jour le statut de la simulation
      await this.updateSimulationStatus(scenarioId, 'failed', 0, error);

      // 4. Envoyer une notification temps réel via Pusher
      const channelName = `private-user-${userId}`;
      await pusherServer.trigger(channelName, SimulationEvent.FAILED, {
        scenarioId,
        scenarioName: scenario.name,
        error,
        timestamp: new Date().toISOString(),
