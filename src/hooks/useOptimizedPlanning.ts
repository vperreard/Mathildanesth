...
/**
 * Hook optimisé pour la gestion du planning avec auto-save et prefetching
 *
 * @description Gère l'état local du planning avec synchronisation automatique vers le serveur.
 * Optimise les performances via debouncing, batching des mises à jour et prefetching intelligent.
 * Gère également la résolution des conflits et la récupération en cas d'erreur.
 *
 * @param {UseOptimizedPlanningProps} props - Configuration du hook
 * @param {Date} [props.month] - Mois à afficher (vue mensuelle)
 * @param {Date} [props.week] - Semaine à afficher (vue hebdomadaire)
 * @param {boolean} [props.autoSave=true] - Active la sauvegarde automatique
 * @param {number} [props.saveDelay=2000] - Délai avant sauvegarde auto (ms)
 * @param {'month' | 'week'} [props.viewType='week'] - Type de vue
 * @param {boolean} [props.enablePrefetch=true] - Active le prefetching des données adjacentes
 *
 * @returns {Object} Objet contenant:
 *   - assignments: Attributions actuelles (avec modifications locales)
 *   - users: Liste des utilisateurs disponibles
 *   - isLoading: État de chargement initial
 *   - isSyncing: État de synchronisation en cours
 *   - error: Erreur éventuelle
 *   - updateAssignment: Fonction pour modifier une attribution
 *   - deleteAssignment: Fonction pour supprimer une attribution
 *   - createAssignment: Fonction pour créer une attribution
 *   - refreshData: Force le rechargement des données
 *   - hasUnsavedChanges: Indique si des changements sont en attente
 *
 * @example
 * const {
 *   assignments,
 ...
 *
 * @performance
 * - Debouncing des mises à jour pour éviter les requêtes excessives
 * - Batching automatique des modifications multiples
 * - Prefetching des semaines adjacentes pour navigation fluide
...
export function useOptimizedPlanning({
  ...
  const updateAssignment = useCallback(
    (assignmentId: string, changes: Partial<Attribution>) => {
  ...
  const moveAssignment = useCallback(
    (assignmentId: string, newUserId: string, newDate: Date) => {
  ...
  return {
    ...
    // Utils
    getAssignmentById: useCallback(
      (id: string) => attributions.find(a => a.id === id),
      [attributions]
    ),
    getUserAssignments: useCallback(
      (userId: string) => attributions.filter(a => a.userId === userId),
      [attributions]
    ),
  };
}
