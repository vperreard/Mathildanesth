...
import { authOptions } from "@/app/api/auth/[...nextauth]/route"; // Chemin direct vers vos authOptions
import { Role } from '@prisma/client'; // Importer Role si ce n'est pas déjà fait

export function handleApiError(error: unknown, defaultMessage: string = "Une erreur est survenue.") {
    // ... existing code ...
}
...
export async function getCurrentUser() {
    ...
    if (session?.user) {
        return session.user as { id: string; name?: string | null; email?: string | null; image?: string | null; role: Role };
    }
