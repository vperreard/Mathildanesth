...
// Type pour l'utilisateur avec les champs sélectionnés
...
async function logUnauthorizedAccess(
  ...
  await auditService.logAccessDenied(userId || undefined, path, {
    reason,
    ipAddress,
    userAgent,
    timestamp: new Date().toISOString(),
...
export function withAuth(config: SecurityConfig = {}) {
  return async (req: NextRequest, context?: NextContext): Promise<NextResponse | Response> => {
    const { requireAuth = true, allowedRoles = [], customCheck, resourceType, action } = config;
...
export const SecurityChecks = {
   ...
   */
  hasAccessToSite: async (context: AuthContext, siteId: string): Promise<boolean> => {
