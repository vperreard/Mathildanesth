...
export class ApiCacheMiddleware {
    ...
    private static async invalidateCache(pathname: string): Promise<void> {
        const invalidationKeys = CACHE_INVALIDATION_ROUTES[pathname as keyof typeof CACHE_INVALIDATION_ROUTES];
...
export function withApiCache<T extends (...args: any[]) => Promise<Response>>(
    handler: T,
