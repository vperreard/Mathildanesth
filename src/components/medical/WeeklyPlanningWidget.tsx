...
export default function WeeklyPlanningWidget({
  userId,
  className,
  mockData,
}: WeeklyPlanningWidgetProps) {
  ...
  useEffect(() => {
    // Ne charger le planning que si nous avons un userId ou des données mock
    if (userId || mockData) {
      loadWeeklyPlanning();
    } else {
      setLoading(false);
  ...
  const loadWeeklyPlanning = async () => {
    try {
      ...
      if (mockData) {
        ...
        try {
          const response = await fetch('/api/mon-planning/semaine', {
            ...
            // Timeout de 10 secondes au lieu de 5
          ...
          // Si l'API planning n'est pas disponible, retourner des données vides
          logger.warn('API planning non disponible, utilisation de données vides', {
            error: fetchError,
          });
          data = { shifts: [], stats: {}, notifications: [] };
        }
      }
      ...
      logger.error('Erreur lors du chargement du planning:', {
        error: errorObj.message,
        stack: errorObj.stack,
      });
      const errorMessage = errorObj.message || 'Impossible de charger votre planning';
      toast({
        title: 'Erreur',
      ...
      >
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className={cn('font-medium text-sm', config.text)}>{shiftLabels[shift.type]}</p>
    ...
    <Card className={cn('w-full', className)}>
      ...
      <CardContent>
        ...
        {nextShift && (
          <div className="mb-6 p-4 bg-primary/10 rounded-lg border border-primary/20">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {shiftLabels[nextShift.type]} -{' '}
              {format(new Date(nextShift.startTime), 'EEEE d MMMM à HH:mm', { locale: fr })}
            </p>
          </div>
        )}
        ...
        <div className="space-y-4">
          {weekData.map(day => (
            ...
            >
              ...
              {day.shifts.length > 0 ? (
                <div className="space-y-2">{day.shifts.map(renderShift)}</div>
              ) : (
                <p className="text-sm text-muted-foreground italic">Aucune affectation</p>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <Button variant="outline" onClick={() => handleQuickAction('conge')} className="w-full">
            <Sun className="h-4 w-4 mr-2" />
            Demander congé
          </Button>
          <Button variant="outline" onClick={() => handleQuickAction('echange')} className="w-full">
            <Users className="h-4 w-4 mr-2" />
            Échanger garde
          </Button>
          <Button variant="outline" onClick={() => handleQuickAction('equipe')} className="w-full">
