...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ children, allowedRoles }) => {
    ...
    logger.info("ProtectedRoute - Check:", { isLoading, user: !!user, userId: user?.id, userRole: user?.role, path: typeof window !== 'undefined' ? window.location.pathname : 'SSR', allowedRoles });

    useEffect(() => {
        // Uniquement agir quand le chargement initial est terminé
        if (!isLoading) {
            if (!user) {
                logger.info('ProtectedRoute Effect: User NULL et !isLoading -> Redirection vers /auth/connexion');
            ...
            // Si l'utilisateur est chargé et que des rôles spécifiques sont requis
                ...
                logger.info(`ProtectedRoute Effect: Rôle non autorisé (${user.role}) -> Redirection vers /`);
    ...
    if (isLoading) {
        logger.info("ProtectedRoute - Rendering: LOADING (isLoading)");
    ...
    if (!user || (allowedRoles && allowedRoles.length > 0 && !allowedRoles.includes(user.role))) {
        logger.info("ProtectedRoute - Rendering: LOADING (user null ou rôle invalide post-isLoading)");
    ...
    logger.info("ProtectedRoute - Rendering: AUTHORIZED - Affichage children");
