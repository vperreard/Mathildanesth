    ...
    persist(
      (set, get) => ({
        ...
        fetchLeaves: async (userId: string) => {
          ...
          try {
            const response = await axios.get(`/api/leaves?userId=${userId}`);
            // Handle paginated response
            const leaves = response.data.items || response.data || [];
            set({ leaves: Array.isArray(leaves) ? leaves : [], isLoading: false });
          } catch (error: unknown) {
            ...
            set({
              ...
              isLoading: false,
              leaves: [], // Ensure leaves is always an array
            });
          }
        },
        ...
        fetchLeaveBalance: async (userId: string, year: number) => {
          ...
          try {
            const response = await axios.get<LeaveBalance>(
              `/api/leaves/balance?userId=${userId}&year=${year}`
            );
        ...
        createLeave: async (leaveData: Partial<Leave>) => {
          ...
          try {
            ...
            const apiPayload = {
              ...
              typeCode: leaveData.typeCode || leaveData.type,
              reason: leaveData.reason,
            };

            logger.info('[leaveStore] Sending API payload:', JSON.stringify(apiPayload, null, 2));

            const response = await axios.post<Leave>('/api/leaves', apiPayload);
        ...
        updateLeave: async (id: string, leaveData: Partial<Leave>) => {
          ...
          try {
            ...
            set({
              error:
                error.response?.data?.error || 'Impossible de mettre à jour la demande de congé.',
              isSubmitting: false,
            });
            throw error;
          }
        ...
        setFilter: (key: keyof LeaveFilters, value: unknown) =>
          set(state => ({
        ...
        checkAllowance: async (userId: string, type: LeaveType, days: number) => {
          set({ isLoading: true, error: null });
          try {
            const response = await axios.post<LeaveAllowanceCheckResult>(
              '/api/leaves/check-allowance',
              {
                userId,
                type,
                days,
