...
export class NotificationService {
  ...
  private notificationsCache: Map<
    string,
    {
  ...
  public async updateConfig(config: Partial<NotificationConfig>): Promise<void> {
    ...
    try {
      ...
      logger.error('Erreur lors de la sauvegarde de la configuration:', error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  private emitEvent(
    type: LeaveNotificationType | string,
    notification: LeaveRelatedNotification
  ): void {
  ...
  public async sendNotification(
    notification: Omit<LeaveRelatedNotification, 'id' | 'createdAt' | 'read'>
  ): Promise<LeaveRelatedNotification> {
    ...
    try {
      ...
      logger.error("Erreur lors de l'envoi de la notification:", error instanceof Error ? error : new Error(String(error)));
  ...
  public async getNotifications(
    ...
    try {
      ...
      const paginatedResult: PaginatedNotificationsResult = {
        ...
        nextOffset:
          offset + notifications.length < totalCount ? offset + notifications.length : undefined,
      };
   ...
   */
  public async loadMoreNotifications(
    userId: string,
    currentOptions: NotificationLoadOptions = {}
  ): Promise<PaginatedNotificationsResult> {
  ...
  public async getUnreadNotifications(
    userId: string,
    options: Omit<NotificationLoadOptions, 'unreadOnly'> = {}
  ): Promise<PaginatedNotificationsResult> {
  ...
  public async markAsRead(notificationId: string): Promise<void> {
    try {
      ...
      for (const [cacheKey, cachedValue] of this.notificationsCache.entries()) {
        ...
        if (
          JSON.stringify(updatedNotifications) !== JSON.stringify(cachedValue.data.notifications)
        ) {
  ...
  public async deleteNotification(notificationId: string): Promise<void> {
    try {
      ...
      for (const [cacheKey, cachedValue] of this.notificationsCache.entries()) {
        const notificationToDelete = cachedValue.data.notifications.find(
          n => n.id === notificationId
        );
        const updatedNotifications = cachedValue.data.notifications.filter(
          n => n.id !== notificationId
        );
  ...
  public async notifyLeaveRequest(leave: Leave, requestor: User): Promise<void> {
    try {
      ...
      logger.error("Erreur lors de l'envoi de la notification de demande de congé:", error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  public async notifyLeaveStatusUpdate(leave: Leave, updatedBy?: User): Promise<void> {
    try {
      ...
      logger.error("Erreur lors de l'envoi de la notification de mise à jour de congé:", error instanceof Error ? error : new Error(String(error)));
    }
  }
    ...
    try {
      ...
      for (const recipientId of schedulerIds) {
        await this.sendNotification({
          ...
          actions: [
            {
              label: 'Gérer le conflit',
              action: 'MANAGE',
              url: `/admin/conges/conflicts?id=${leave.id}`,
            },
          ],
        });
      }
    } catch (error: unknown) {
      logger.error("Erreur lors de l'envoi de la notification de conflit de congés:", error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  public async sendLeaveReminder(leave: Leave, daysUntilStart: number): Promise<void> {
    try {
      ...
      logger.error("Erreur lors de l'envoi du rappel de congé:", error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  public async notifyQuotaLow(
    ...
    try {
      ...
      logger.error("Erreur lors de l'envoi de la notification de quota bas:", error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  private async getSchedulerManagerIds(): Promise<string[]> {
    try {
      ...
      logger.error('Erreur lors de la récupération des gestionnaires de planning:', error instanceof Error ? error : new Error(String(error)));
      return [];
    }
  }
