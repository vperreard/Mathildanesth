...
import { calculateEaster, parseDate } from '@/utils/dateUtils';
...
class PublicHolidayService {
  ...
  // Durée de vie du cache côté client: 1 heure (plus courte pour éviter l'accumulation)
  private readonly DEFAULT_CACHE_TTL = 60 * 60 * 1000;

  // Cache des jours fériés par année
  private holidayCache: Map<
    string,
    {
      data: PublicHoliday[];
      timestamp: number;
  ...
  private constructor(options?: PublicHolidayServiceOptions) {
    this.options = {
      ...
      preloadData: options?.preloadData ?? false, // Désactivé par défaut pour les performances
      preloadYears: options?.preloadYears ?? [0, 1], // Réduire le préchargement
    };

    // Préchargement désactivé pour améliorer les performances côté client
    // Les données seront chargées à la demande
    if (this.options.preloadData && typeof window === 'undefined') {
      // Précharger seulement côté serveur
      this.preloadData();
    }
  }
  ...
  private async preloadData(): Promise<void> {
    try {
      ...
      await Promise.all(yearsToLoad.map(year => this.getPublicHolidaysForYear(year)));
    } catch (error: unknown) {
      logger.error('[PublicHolidayService] Erreur lors du préchargement des données:', {
        error: error,
      });
    }
  }
  ...
  private generateCacheKey(filter: PublicHolidayFilter | number): string {
    ...
    if (filter.endDate) {
      const date =
        typeof filter.endDate === 'string' ? filter.endDate : format(filter.endDate, 'yyyy-MM-dd');
      parts.push(`end:${date}`);
    }
  ...
  public async getPublicHolidaysForYear(year: number): Promise<PublicHoliday[]> {
    ...
    try {
      ...
      logger.warn(
        `[PublicHolidayService] Fallback to local calculation for year ${year}: ${errorMessage}`
      );
  ...
  public async getPublicHolidays(filter: PublicHolidayFilter = {}): Promise<PublicHoliday[]> {
    ...
    if (filter.startDate) {
      const startDate =
        typeof filter.startDate === 'string' ? parseISO(filter.startDate) : filter.startDate;
      startYear = getYear(startDate);
    }

    if (filter.endDate) {
      const endDate =
        typeof filter.endDate === 'string' ? parseISO(filter.endDate) : filter.endDate;
    ...
    if (filter.region) {
      holidays = holidays.filter(holiday => holiday.regions?.includes(filter.region as string));
    }

    if (filter.country) {
      holidays = holidays.filter(holiday => holiday.country === filter.country);
    }
  ...
  public async getPublicHolidaysInRange(
    ...
    try {
      ...
      // Utiliser l'API au lieu de Prisma côté client avec timeout
      ...
      const response = await apiClient.get(`/api/public-holidays?${params.toString()}`, {
        signal: controller.signal,
      });

      clearTimeout(timeoutId);
      const result = response.data as PublicHoliday[];
      ...
      logger.error(
        '[PublicHolidayService] Erreur lors de la récupération des jours fériés dans une plage:',
        {
          error: error,
      ...
      if (cachedEntry) {
        logger.info('[PublicHolidayService] Utilisation du cache expiré en fallback');
        return cachedEntry.data;
      }

      // Si pas de cache, utiliser les jours fériés par défaut
      logger.info('[PublicHolidayService] Utilisation du calcul local en fallback');
      return this.filterDefaultHolidaysByRange(startDate, endDate, region);
    }
  }

  /**
   * Mapper un objet Prisma vers le format PublicHoliday
   * @private
   */
  private mapPrismaToPublicHoliday(prismaHoliday: any): PublicHoliday {
    return {
      id: prismaHoliday.id,
      date:
        typeof prismaHoliday.date === 'string'
          ? prismaHoliday.date
          : format(prismaHoliday.date, 'yyyy-MM-dd'),
      name: prismaHoliday.name,
      description: prismaHoliday.description,
      isNational: prismaHoliday.isNational || true,
      region: prismaHoliday.region,
      country: prismaHoliday.country || 'FR',
      isWorkingDay: prismaHoliday.isWorkingDay || false,
    };
  }
  ...
  public async createPublicHoliday(data: CreatePublicHolidayDTO): Promise<PublicHoliday> {
    try {
      const formattedDate =
        typeof data.date === 'string' ? data.date : format(data.date, 'yyyy-MM-dd');
      ...
      try {
        ...
        logger.warn(
          "[PublicHolidayService] Impossible de créer le jour férié via l'API. Utilisation du stockage local.",
          apiError
        );
      ...
      logger.error('[PublicHolidayService] Erreur lors de la création du jour férié:', {
        error: error,
      });
      throw error;
    }
  ...
  public async updatePublicHoliday(data: UpdatePublicHolidayDTO): Promise<PublicHoliday | null> {
    try {
      ...
      if (data.date) {
        formattedDate = typeof data.date === 'string' ? data.date : format(data.date, 'yyyy-MM-dd');
      }
      ...
      try {
        ...
        logger.warn(
          "[PublicHolidayService] Impossible de mettre à jour le jour férié via l'API. Utilisation du stockage local.",
          apiError
        );
        ...
        const allHolidays = Array.from(this.holidayCache.values()).flatMap(entry => entry.data);

        return allHolidays.find(holiday => holiday.id === data.id) || null;
      }
    } catch (error: unknown) {
      logger.error('[PublicHolidayService] Erreur lors de la mise à jour du jour férié:', {
        error: error,
      });
      throw error;
    }
  ...
  public async deletePublicHoliday(id: string): Promise<boolean> {
    try {
      ...
      try {
        ...
        logger.warn(
          "[PublicHolidayService] Impossible de supprimer le jour férié via l'API. Utilisation du stockage local.",
          apiError
        );
      ...
      logger.error('[PublicHolidayService] Erreur lors de la suppression du jour férié:', {
        error: error,
      });
      throw error;
    }
  ...
  public async importPublicHolidays(holidays: CreatePublicHolidayDTO[]): Promise<PublicHoliday[]> {
    ...
    for (const holidayData of holidays) {
      try {
        ...
        logger.error(
          `[PublicHolidayService] Erreur lors de l'importation du jour férié:`,
          holidayData,
