...
export class QuotaApiService {
     ...
     */
    async simulateCarryOver(request: QuotaCarryOverCalculationRequest): Promise<QuotaCarryOverCalculationResult> {
     ...
     */
    async executeCarryOver(request: QuotaCarryOverCalculationRequest): Promise<QuotaCarryOverCalculationResult> {
    ...
    async getAvailableSourceTypes(userId: string): Promise<LeaveType[]> {
        const response = await this.apiClient.get(`${this.baseUrl}/transfers/available-source-types/${userId}`);
        return response.data;
    }
    ...
    async getAvailableTargetTypes(userId: string, sourceType: LeaveType): Promise<LeaveType[]> {
        const response = await this.apiClient.get(`${this.baseUrl}/transfers/available-target-types/${userId}`, {
            params: { sourceType }
        });
        return response.data;
    }
     ...
     */
    async getEligibleCarryOverTypes(userId: string, fromYear: number, toYear: number): Promise<LeaveType[]> {
        const response = await this.apiClient.get(`${this.baseUrl}/carry-overs/eligible-types/${userId}`, {
     ...
     */
    async updateTransferRule(ruleId: string, rule: Partial<QuotaTransferRule>): Promise<QuotaTransferRule> {
     ...
     */
    async updateCarryOverRule(ruleId: string, rule: Partial<QuotaCarryOverRule>): Promise<QuotaCarryOverRule> {
    ...
    async approveTransfer(transferId: string, comments?: string): Promise<QuotaTransferResult> {
        const response = await this.apiClient.post(`${this.baseUrl}/transfers/${transferId}/approve`, { comments });
        return response.data;
    }
    ...
    async rejectTransfer(transferId: string, comments?: string): Promise<QuotaTransferResult> {
        const response = await this.apiClient.post(`${this.baseUrl}/transfers/${transferId}/reject`, { comments });
        return response.data;
    }
     ...
     */
    async approveCarryOver(carryOverId: string, comments?: string): Promise<QuotaCarryOverCalculationResult> {
        const response = await this.apiClient.post(`${this.baseUrl}/carry-overs/${carryOverId}/approve`, { comments });
        return response.data;
    }
     ...
     */
    async rejectCarryOver(carryOverId: string, comments?: string): Promise<QuotaCarryOverCalculationResult> {
        const response = await this.apiClient.post(`${this.baseUrl}/carry-overs/${carryOverId}/reject`, { comments });
        return response.data;
    }
     ...
     */
    async adjustQuota(quotaId: string, adjustment: Omit<QuotaAdjustment, 'id' | 'quotaId' | 'timestamp'>): Promise<EmployeeQuota> {
    ...
    async cancelTransfer(transferId: string, reason: string): Promise<QuotaTransferResult> {
        const response = await this.apiClient.post(`${this.baseUrl}/transfers/cancel/${transferId}`, { reason });
        return response.data;
    }
     ...
     */
    async getApplicableTransferRules(sourceType: LeaveType, targetType: LeaveType): Promise<QuotaTransferRule[]> {
