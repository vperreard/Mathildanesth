import { renderHook, act } from '@testing-library/react'; // Utiliser react-hooks si ancienne version
...
describe('useLeave Hook', () => {
    ...
    it('should initialize with null leave and empty leaves array when no initialLeave provided', () => {
    ...
    describe('checkConflicts', () => {
        ...
        it('should call checkLeaveConflicts service with Date objects and update state on success', async () => {
            ...
            // Pour tester l'état de loading pendant l'appel
            let loadingDuringCall = false;
            mockedCheckLeaveConflicts.mockImplementationOnce(async () => {
                // Capturer l'état de loading pendant l'appel
                loadingDuringCall = result.current.loading;
                return mockConflictResultEmpty;
            });
            ...
            // Vérifier que loading était true pendant l'appel
        ...
        it('should return null if userId is missing', async () => {
            // Utiliser un mock qui renvoie un utilisateur null ou undefined
            jest.mock('@/context/AuthContext', () => ({
                useAuth: () => ({
                    user: null
                })
            ...
            const { result } = renderHook(() => useLeave({
                initialLeave: mockInitialLeave,
                // Pas de userId ici, on dépend de useAuth qui retourne null
            }));
            ...
            // Vérifier que setError a été appelé avec les informations appropriées
            expect(mockedSetError).toHaveBeenCalledWith(
                'conflicts',
                expect.objectContaining({
                    message: expect.stringContaining('Utilisateur non connecté'),
                    severity: 'warning'
                })
            );

            // Vérifier la valeur de retour null
            expect(returnedResult).toBeNull();
        });
    ...
    describe('fetchUserLeaves', () => {
        ...
        it('should set error state if fetchLeaves service fails', async () => {
            ...
            mockedFetchLeaves.mockClear(); // Réinitialiser le compteur d'appels
            ...
            expect(result.current.leaves).toEqual([]); // Liste reste vide
        });
    });
    ...
    describe('fetchLeaveDetails', () => {
        ...
        it('should set error state if fetchLeaveById service fails', async () => {
            ...
            expect(result.current.leave).toBeNull(); // leave ne doit pas être mis à jour
        });
    });
