...
export const useCalendarQuery = (params: CalendarQueryParams) => {
    ...
    const { data: events = [], isLoading, error } = useQuery({
        queryKey,
        queryFn: async () => {
            const response = await axios.get('/api/calendrier/events', { params });
    ...
    const updateEventMutation = useMutation({
        mutationFn: async (event: AnyCalendarEvent) => {
            const response = await axios.put(`/api/calendrier/events/${event.id}`, event);
    ...
    const createEventMutation = useMutation({
        mutationFn: async (event: Omit<AnyCalendarEvent, 'id'>) => {
            const response = await axios.post('/api/calendrier/events', event);
            return response.data;
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey });
    ...
    const deleteEventMutation = useMutation({
        mutationFn: async (eventId: string) => {
            await axios.delete(`/api/calendrier/events/${eventId}`);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey });
        },
