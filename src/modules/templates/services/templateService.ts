...
const mapAffectationModeleDtoToTemplateAffectation = (
  affectationDto: unknown
): TemplateAffectation => {
  const typeAffectation =
    affectationDto.activityType?.code || affectationDto.activityType?.name || 'INCONNU';
  let postesCalculated =
    affectationDto.personnelRequis?.reduce(
      (sum: number, pr: unknown) => sum + (pr.nombreRequis || 0),
      0
    ) || 0;
  ...
  const frontendJourSemaine =
    invertedDayOfWeekMapping[
      affectationDto.jourSemaine.toUpperCase() as keyof typeof invertedDayOfWeekMapping
    ] || (affectationDto.jourSemaine as DayOfWeek);
  ...
  const configuration: AffectationConfiguration = {
    ...
    nom:
      affectationDto.detailsJson?.frontendConfigNom ||
      affectationDto.activityType?.name ||
      'Configuration par défaut',
    postes:
      affectationDto.detailsJson?.frontendPostes ||
      affectationDto.personnelRequis?.map((pr: unknown, index: number) => ({
        ...
        status:
          pr.detailsJson?.frontendStatut ||
          (pr.nombreRequis && pr.nombreRequis > 0 ? 'REQUIS' : 'INDISPONIBLE'),
  ...
  return {
    ...
    ouvert: estOuvert,
    postesRequis:
      affectationDto.detailsJson?.frontendPostesRequis !== undefined
        ? affectationDto.detailsJson?.frontendPostesRequis
        : postesCalculated,
...
const mapTrameModeleDtoToPlanningTemplate = (dto: unknown): PlanningTemplate => {
  ...
  if (variations.length) {
    logger.info(
      `[Modèle Service] ${variations.length} variations trouvées dans detailsJson pour la trameModele ${dto.id}`
    );
  }
  ...
  return {
    ...
    // Preserve the entire detailsJson object for room/sector selections and other data
...
export const templateService = {
  async getTemplates(): Promise<PlanningTemplate[]> {
    logger.info(
      '[Modèle Service] Appel de getTemplates via API /api/trame-modeles?includeAffectations=true...'
    );
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(
          '[Modèle Service] Erreur API lors de getTemplates:',
          response.status,
          errorData
        );
        throw new Error(
          `Erreur API (${response.status}): ${errorData.error || 'Impossible de récupérer les trameModeles'}`
        );
      }
      ...
      if (!Array.isArray(dtos)) {
        logger.error(
          "[Modèle Service] ERREUR : la réponse de l'API n'est pas un array! Reçu:",
          dtos
        );
        return [];
      }
      ...
      if (error instanceof Error) {
        ...
        throw new Error(
          'Une erreur inconnue est survenue lors de la récupération des trameModeles.'
        );
      }
    }
  },
  ...
  async getTemplateById(id: string): Promise<PlanningTemplate | null> {
    logger.info(
      `[Modèle Service] Fetching modèle by ID: ${id} via API /api/trameModele-modeles/${id}?includeAffectations=true...`
    );
    try {
      const token = await getClientAuthToken();
      ...
      if (!response.ok) {
        ...
        const errorData = await response.json().catch(() => ({ error: response.statusText }));
        logger.error(
          '[Modèle Service] Erreur API lors de getTemplateById:',
          response.status,
          errorData
        );
        throw new Error(
          `Erreur API (${response.status}): ${errorData.error || 'Impossible de récupérer la trameModele'}`
        );
      }
      ...
      if (error instanceof Error) {
        ...
        throw new Error(
          'Une erreur inconnue est survenue lors de la récupération de la trameModele.'
        );
      }
    }
  },
  ...
  async saveTemplate(
    ...
    logger.info(
      `[Modèle Service] Appel de saveTemplate pour: ${templateData.nom} via API /api/trameModele-modeles...`
    );
    ...
    const tramePayload = {
      ...
      dateDebutEffet: templateData.dateDebutEffet
        ? new Date(templateData.dateDebutEffet).toISOString()
        : new Date().toISOString(),
      dateFinEffet: templateData.dateFinEffet
        ? new Date(templateData.dateFinEffet).toISOString()
        : null,
      recurrenceType: templateData.recurrenceType || 'HEBDOMADAIRE',
      joursSemaineActifs:
        templateData.joursSemaineActifs && templateData.joursSemaineActifs.length > 0
          ? templateData.joursSemaineActifs
          : [1, 2, 3, 4, 5],
      typeSemaine: templateData.typeSemaine || 'TOUTES', // Assurer une valeur par défaut
      roles:
        templateData.roles && templateData.roles.length > 0 ? templateData.roles : [RoleType.TOUS],
              ...
              frontendVariations: templateData.variations.map(v => {
                ...
                if (v.configuration) {
                  const sanitizedConfig = {
                    ...
                    postes: Array.isArray(v.configuration.postes)
                      ? v.configuration.postes.map(p => ({
                          id:
                            p.id ||
                            `poste_var_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error ||
            `Erreur API (${response.status}): Impossible de sauvegarder les métadonnées de la trameModele.`
        );
      }
      savedOrUpdatedTrameModeleDto = await response.json();
      trameModeleId = String(savedOrUpdatedTrameModeleDto.id); // Obtenir l'ID réel
      logger.info(
        `[Modèle Service] Métadonnées de la trameModele ${isCreating ? 'créées' : 'mises à jour'} avec succès. ID: ${trameModeleId}`
      );
    } catch (error: unknown) {
      logger.error(
        `[Modèle Service] Erreur lors de la sauvegarde des métadonnées de la tableau de service:`,
        { error: error }
      );
    ...
    try {
      ...
      const existingAffectationsResponse = await fetch(
        `${API_BASE_URL}/${trameModeleId}/gardes/vacations`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      if (!existingAffectationsResponse.ok)
        throw new Error('Impossible de récupérer les gardes/vacations existantes.');
      ...
      const deletePromises = existingAffectations.map(aff =>
        fetch(`${API_BASE_URL}/../affectation-modeles/${aff.id}`, {
          // Note: L'URL de suppression est différente
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        }).then(res => {
          if (!res.ok && res.status !== 404) {
            // Ignorer 404 si déjà supprimé
      ...
      logger.info(
        `[Modèle Service] ${existingAffectations.length} gardes/vacations existantes supprimées pour la trameModele ${trameModeleId}.`
      );

      // 3. Créer les nouvelles gardes/vacations
      logger.info(
        `[Modèle Service] Tentative de création de ${templateData.affectations.length} nouvelles gardes/vacations pour la trameModele ${trameModeleId}...`
      );
      ...
      for (const affectation of templateData.affectations) {
        ...
        if (!affectationDto) {
          logger.warn(
            `[Modèle Service] Impossible de mapper l'affectation (jour: ${affectation.jour}, type: ${affectation.type}), elle sera ignorée.`
          );
          continue;
        }
        ...
        if (!createResponse.ok) {
          ...
          logger.error(
            `Échec de la création de l'affectation pour type '${affectation.type}': ${createResponse.status}`,
            errBody
          );
      ...
      logger.info(
        `[Modèle Service] ${createdAffectations.length} nouvelles gardes/vacations créées pour la trameModele ${trameModeleId}.`
      );

      // Log pour débugage des gardes/vacations et postes
      logger.info(
        `[Modèle Service DEBUG] Gardes/Vacations sauvegardées:`,
        createdAffectations.length
      );
      logger.info(
        `[Modèle Service DEBUG] Exemple première affectation si existe:`,
        createdAffectations.length > 0
          ? JSON.stringify(createdAffectations[0], null, 2)
          : 'aucune affectation'
      );
      ...
      if (
        ...
        Object.keys(affectationIdMapping).length > 0
      ) {
        logger.info(
          `[Modèle Service] Mise à jour des références d'affectationId dans ${templateData.variations.length} variations...`
        );
        ...
        if (!trameResponse.ok) {
          logger.error(
            `[Modèle Service] Erreur lors de la récupération de la trameModele pour mise à jour des variations:`,
            await trameResponse.text()
          );
        } else {
          ...
          if (!updateResponse.ok) {
            logger.error(
              `[Modèle Service] Erreur lors de la mise à jour des références d'affectationId dans les variations:`,
              await updateResponse.text()
            );
          } else {
            logger.info(
              `[Modèle Service] Références d'affectationId mises à jour avec succès dans les variations.`
            );
          }
        }
      }
      ...
      logger.error(
        `[Modèle Service] Erreur lors de la gestion des gardes/vacations pour la trameModele ${trameModeleId}:`,
        { error: error }
      );
      ...
      throw new Error(
        `Erreur lors de la sauvegarde des affectations: ${error instanceof Error ? error.message : String(error)}`
      );
    }

    // --- Étape 3: Sauvegarder les variations (extensions des gardes/vacations) ---
    logger.info(
      `[Modèle Service] ${templateData.variations?.length || 0} variations incluses dans le payload initial de la trameModele ${trameModeleId}.`
    );
    ...
    if (!finalTrame) {
      ...
      logger.error(
        `[Modèle Service] CRITICAL: La trameModele ${trameModeleId} sauvegardée n'a pas pu être récupérée après la gestion des gardes/vacations.`
      );
    ...
    logger.info(
      `[Modèle Service DEBUG] Tableau de service chargée avec ${finalTrame.affectations.length || 0} gardes/vacations.`
    );
    logger.info(
      `[Modèle Service DEBUG] Exemple première affectation si existe:`,
      finalTrame.affectations.length
        ? JSON.stringify(finalTrame.affectations[0], null, 2)
        : 'aucune affectation'
    );
    return finalTrame;
  },
  ...
  async deleteTemplate(id: string): Promise<void> {
    logger.info(
      `[Modèle Service] Appel de deleteTemplate pour ID: ${id} via API /api/trameModele-modeles...`
    );
    try {
      const token = await getClientAuthToken();
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: response.statusText }));
        logger.error(
          '[Modèle Service] Erreur API lors de deleteTemplate:',
          response.status,
          errorData
        );
        throw new Error(
          `Erreur API (${response.status}): ${errorData.error || 'Impossible de supprimer la trameModele'}`
        );
      }
      ...
      if (error instanceof Error) {
        ...
        throw new Error(
          'Une erreur inconnue est survenue lors de la suppression de la trameModele.'
        );
      }
    }
  },
  ...
  async duplicateTemplate(
    id: string,
    availableFullActivityTypes: FullActivityType[]
  ): Promise<PlanningTemplate> {
    ...
    const trameAvecAffectationsDupliquees: PlanningTemplate = {
      ...duplicatedMetaData,
      affectations: (originalTemplate.affectations || []).map(aff => ({
        ...aff,
        id: `temp_dup_${Date.now()}_${Math.random()}`,
      })),
      variations: (originalTemplate.variations || []).map(v => ({
        ...v,
        id: `temp_var_dup_${Date.now()}_${Math.random()}`,
      })),
    };
  ...
  async getAvailableAffectationTypes(): Promise<FullActivityType[]> {
    // MODIFIÉ: Type de retour
    logger.info(
      '[Modèle Service] Appel de getAvailableAffectationTypes via API /api/activity-types...'
    );

    // Types d'activité par défaut en cas d'erreur API
    const fallbackActivityTypes: FullActivityType[] = [
      { id: 'activity1', name: 'Bloc Opératoire', code: 'BLOC', category: 'STANDARD' },
      { id: 'activity2', name: 'Consultation', code: 'CONSULT', category: 'CONSULTATION' },
      { id: 'activity3', name: 'Garde', code: 'GARDE', category: 'GARDE' },
      { id: 'activity4', name: 'Astreinte', code: 'ASTREINTE', category: 'ASTREINTE' },
    ];
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: response.statusText }));
        logger.error(
          '[Modèle Service] Erreur API lors de getAvailableAffectationTypes:',
          response.status,
          errorData
        );
        logger.warn(
          "[Modèle Service] Utilisation des types d'activité par défaut suite à l'erreur API"
        );
        return fallbackActivityTypes;
      }
      ...
      if (!Array.isArray(activityTypesFromApi)) {
        logger.error(
          "[Modèle Service] ERREUR : la réponse de /api/activity-types n'est pas un array! Reçu:",
          activityTypesFromApi
        );
        logger.warn(
          "[Modèle Service] Utilisation des types d'activité par défaut suite à l'erreur de format"
        );
        return fallbackActivityTypes;
      }

      // Mapper les données de l'API au format attendu
      const mappedActivityTypes = activityTypesFromApi.map(at => ({
        id: at.id || `fallback-${at.name?.toLowerCase().replace(/\s+/g, '-')}`,
        name: at.name || "Type d'activité",
        code: at.code || at.name?.toUpperCase().slice(0, 6) || 'UNKNOWN',
        category: at.category || 'AUTRE',
      }));

      logger.info("[Modèle Service] Types d'activité (complets) récupérés:", mappedActivityTypes);
      return mappedActivityTypes;
    } catch (error: unknown) {
      logger.error('[Modèle Service] Erreur DANS getAvailableAffectationTypes (API):', {
        error: error,
      });
      logger.warn(
        "[Modèle Service] Utilisation des types d'activité par défaut suite à l'erreur",
...
const mapTemplateAffectationToApiDto = (
  ...
  const activityType = availableActivityTypes.find(
    (at: FullActivityType) => at.code === affectation.type
  );
  if (!activityType) {
    logger.error(
      `[Modèle Service] mapTemplateAffectationToApiDto: ActivityType avec le code '${affectation.type}' non trouvé.`
    );
    return null;
  }

  const mappedJourSemaine =
    dayOfWeekMapping[affectation.jour.toUpperCase() as keyof typeof dayOfWeekMapping];
  if (!mappedJourSemaine) {
    logger.error(
      `[Modèle Service] mapTemplateAffectationToApiDto: Jour de la semaine invalide ou non mappé: '${affectation.jour}'`
    );
    return null;
  }

  // Calculer le nombre total de postes requis
  const totalPostesRequis =
    affectation.configuration?.postes
      ?.filter(p => p.quantite > 0 && p.status !== 'INDISPONIBLE')
      .reduce((total, poste) => total + poste.quantite, 0) || 0;

  const nombrePostesRequis =
    totalPostesRequis > 0 ? totalPostesRequis : affectation.ouvert ? affectation.postesRequis : 0;
  ...
  logger.info(
    `[Modèle Service DEBUG] personnelRequisConfig simplifié avant création du DTO:`,
    JSON.stringify(personnelRequis, null, 2)
  );
  logger.info(
    `[Modèle Service DEBUG] Nombre total de postes requis calculé: ${nombrePostesRequis}`
  );
  ...
  const dto: AffectationModeleCreateDto = {
    ...
    operatingRoomId: affectation.configuration?.emplacementPhysique
      ? parseInt(affectation.configuration.emplacementPhysique, 10)
      : undefined, // Supposant que salleId est dans emplacementPhysique
 ...
 */
const mapVariationToApiDto = (variation: ConfigurationVariation, trameId: string): any => {
